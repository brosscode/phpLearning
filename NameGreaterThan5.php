...
// template
$sql = "SELECT * FROM users WHERE LENGTH(name) > 5  AND name LIKE ? AND email LIKE ? AND zip LIKE ? ORDER BY accountCreated ASC";
...
if (!mysqli_stmt_prepare($stmt, $sql)) {
  ...
  echo "<li>" . "Name: " . $row["name"] . " | Email: " . $row["email"] . " | Zip: " . $row["zip"] . " | Created: " . $row["accountCreated"] . "</li>";
  }
  echo "</ul>";
  // close statment
  $stmt->close();
